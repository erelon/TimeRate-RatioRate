...
ax.plot(sin_line, label=f"SinLogD (offset=15, log_scale={log_scale})", color="orange")
ax.plot(cos_line, label=f"CosLogD (offset=10, log_scale={log_scale/2})", color="blue")

division = np.divide(sin_line, cos_line)
ax.plot(division, label="SinLogD / CosLogD", color="red")

ax.set_xlabel("Step")
ax.set_ylabel("Value")
ax.set_title("Drifting Sin Log, Cos Log, and Ratio")
ax.legend()
ax.grid()
