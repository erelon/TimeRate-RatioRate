...
HarmonicROLAgent, SMARTEMARLAgent, and SMARTRLAgent, and measures what
...
from agents.smart_r import SMARTEMARLAgent, SMARTRLAgent
...
# ── agents to sweep ─────────────────────────────────────────────────────────
AGENT_CLASSES = {
    "Harmonic R":   HarmonicROLAgent,
    "RelaxedSMART": SMARTEMARLAgent,
    "SMART":        SMARTRLAgent,
}
...
def plot_alpha_beta_grouped(
    ...
    Groups: one clustered bar per agent  (HarmonicROL / RelaxedSMART / SMART)
    ...
    # ── colours per agent ────────────────────────────────────────────────────
    agent_colors = {
        agents[0]: "#1a1a1a",   # near-black  → HarmonicROL
        agents[1]: "#fdb462",   # orange      → RelaxedSMART
        agents[2]: "#ef3b2c",   # red         → SMART
    }
    # extend gracefully if more agents are present
    extra = ["#6a51a3", "#31a354", "#2171b5"]
    for j, ag in enumerate(agents[3:], start=0):
        agent_colors[ag] = extra[j % len(extra)]
...
def plot_success_rate_vs_env(
    ...
    styles = [
        dict(color="#1a1a1a", marker="o", ls="-",  lw=2.5),   # Harmonic R
        dict(color="#e08c00", marker="s", ls="--", lw=2.5),   # RelaxedSMART
        dict(color="#ef3b2c", marker="^", ls="-.", lw=2.5),   # SMART
        dict(color="#6a51a3", marker="D", ls=":",  lw=2.5),
